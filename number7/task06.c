...
int solve6 (const char* name1, double* a, int n);
...
int solve6 (const char* name1, double* a, int n){
    int ans = 0, f = 0;
    double prev = 0, current_number = 0;
    FILE* fp;
    if (n == 0){
        return 0;
    }
    ...
    while (fscanf(fp, "%lf", &current_number) == 1){
        if (f != 0){
            if (equal(prev, current_number)){
                ans = maybe_insert(a, ans, n, current_number);
            }
        }
        else{
            f = 1;
