...
void convert_record_to_str(char* res, record& rec, command& cmd);
                        ...
                        switch (cmd.get_command_type()){
                            ...
                            case command_type::select:
                                ...
                                for (;curr;curr = curr->get_next()){
                                    convert_record_to_str(buf, *curr->get_data(), cmd);
                                    writeToClient(i, buf);
                                    curr->get_data()->set_was_used(false);
                                }
                                break;
                            default:
                                break;
...
void convert_record_to_str(char* res, record& rec, command& cmd){
    ...
    const ordering * p = cmd.get_ordering();
    int curr = 0;
    int l = 0;
    for (int i = 0; i < max_items; i++){
        switch (p[i]){
            case ordering::name:
                l = sprintf (res + curr, " %s", rec.get_name()); break;
            case ordering::phone:
                l = sprintf (res + curr, " %d", rec.get_phone()); break;
            case ordering::group:
                l = sprintf (res + curr, " %d", rec.get_group()); break;
            case ordering::none:
                l = 0;
                continue;
        }
