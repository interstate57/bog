...
io_status solve6(char* name1, char* name2, char* name3, char* spaces, int* res);

static void insert_words(std::string line, const char* spaces, std::map<std::string, int>& container){
    size_t index = 0;
    std::string word;
    while ((index = line.find_first_of(spaces)) != std::string::npos){
        word = line.substr(0, index);
        container.insert(std::make_pair(word, 0));
        line = line.substr(index + 1);
    }
    container.insert(std::make_pair(line, 0));
}
...
int main(int argc, char* argv[]){
    ...
    spaces = argv[4];
    t = clock();
    io_status st = solve6(name1, name2, name3, spaces, &res);
