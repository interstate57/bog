...
void solve6(char* name1, char* name2, char* name3, char* spaces, int* res);
...
int main(int argc, char* argv[]){
    ...
    spaces = argv[4];
    t = clock();
    solve6(name1, name2, name3, spaces, &res);
...
void solve6(char* name1, char* name2, char* name3, char* spaces, int* res){
    ...
    while(!ifs2.eof()) {
        ...
        while ((index = line.find(spaces)) != std::string::npos){
            ...
            if (std::find(container.begin(), container.end(), word) != container.end()){
                
                cnt += 1;
            }
            line = line.substr(index + 1);
        }
        if (std::find(container.begin(), container.end(), line) != container.end()){
            cnt += 1;
        }
        *res += cnt;
